// Diatheke 2.0 by Chris Little <chrislit@gotjesus.org>
...
int main(int argc, char **argv)
{
	...
	if (argc > 1) {
	  ...
	  if (argc > 4) {
	    if (!strcmp(argv[4], "plain")) {
	      filter = new ThMLPlain();
	    }
	    else if (!strcmp(argv[4], "html")) {
	      filter = new ThMLHTML();
	    }
	    else if (!strcmp(argv[4], "gbf")) {
	      filter = new ThMLGBF();
	    }
	    else if (!strcmp(argv[4], "rtf")) {
	      filter = new ThMLRTF();
